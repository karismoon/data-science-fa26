...
```{r q1-task}
diamonds %>%
  mutate(carat_cut = cut_interval(carat, 5)) %>% 
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```

**Observations**
- The whiskers and outliers always go up to the max price, no matter the cut category.
- Depending on how the data is grouped, the median price typically consistently goes up with the carat cut, but not always. Same for the quantiles.
